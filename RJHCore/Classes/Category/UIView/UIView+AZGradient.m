...
@property (strong, nonatomic) CAGradientLayer * az_gradientLayer;
...
- (void)az_setMaskGradientBackgroundWithColors:(NSArray<UIColor *> *)colors locations:(NSArray<NSNumber *> *)locations startPoint:(CGPoint)startPoint endPoint:(CGPoint)endPoint {
    ...
    self.az_gradientLayer = [CAGradientLayer layer];
    self.az_gradientLayer.frame = self.bounds;
...
- (CAGradientLayer *)az_gradientLayer {
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setAz_gradientLayer:(CAGradientLayer *)gradientLayer {
    objc_setAssociatedObject(self, @selector(az_gradientLayer), gradientLayer, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    
    self.layer.mask = self.az_gradientLayer;
}
...
- (void)setAz_colors:(NSArray *)colors {
    ...
    if (self.az_gradientLayer) {
        [self.az_gradientLayer setColors:self.az_colors];
    }else {
...
- (void)setAz_locations:(NSArray<NSNumber *> *)locations {
    ...
    if (self.az_gradientLayer) {
        [self.az_gradientLayer setLocations:self.az_locations];
    }else {
        if ([self.layer isKindOfClass:[CAGradientLayer class]]) {
...
- (void)setAz_startPoint:(CGPoint)startPoint {
    ...
    if (self.az_gradientLayer) {
        [self.az_gradientLayer setStartPoint:self.az_startPoint];
    }else {
        if ([self.layer isKindOfClass:[CAGradientLayer class]]) {
...
- (void)setAz_endPoint:(CGPoint)endPoint {
    ...
    if (self.az_gradientLayer) {
        [self.az_gradientLayer setEndPoint:self.az_endPoint];
    }else {
        if ([self.layer isKindOfClass:[CAGradientLayer class]]) {
